...
class TableLimitTest(unittest.TestCase):
    def test_원문이_실제로_쓴_최대_줄_폭이_한계다(self):
        self.assertEqual(kowidth.table_limit(["가나\n다라마", "바"]), 6)

    def test_이름_읽기_색인은_빼고_센다(self):
        self.assertEqual(kowidth.table_limit(["가나다라마바／かな", "가"]), 2)

    def test_원문이_없으면_0(self):
        self.assertEqual(kowidth.table_limit([]), 0)

    def test_한계를_정한_줄이_몇_개인지_센다(self):
        texts = ["가나\n다라", "가나"]
        self.assertEqual(kowidth.limit_samples(texts, 4), 3)
        self.assertEqual(kowidth.limit_samples(["가나다", "가"], 6), 1)
